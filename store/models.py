...
class Esame(models.Model):
    ...
    STATO_CHOICES = [
        (0, 'Prenotato'),  # Collegamento con l'agenda di prenotazione, fino a che lo stato è prenotato il codice esame non viene generato
        (1, 'Accettato'),  # Tutte le informazioni anagrafico-cliniche sono state raccolte, non è ancora stata identificata la data-ora di esecuzione
        ...
        (4, 'Completato'),  # La procedura diagnostica si è conclusa
        (5, 'Trascritto'),  # Lo stato di trascritto indica che nel sistema è presente il testo del referto, anche se non ancora completo
        ...
        (8, 'Annullato'),  # Lo stato di annullato viene utilizzato per invalidare un esame che ha uno stato di accettato o superiore, dato che
                            # per motivi medico-legali non è possibile perdere traccia dell'attività svolta, anche se non è stata portata a compimento
    ]
    ...
    classe_dose_cumulativa = models.IntegerField(default=0)  # Dose totale di radiazioni a cui il paziente è esposto, secondo normativa vigente
...
class Revisione(models.Model):
    ...
    numero_revisione = models.IntegerField(blank=True,
                                           null=True)  # se esiste, la numero O è la copia originale del referto (quindi con 1 revisione ci sono almeno 2 record)
